...
proc ::pool::getopt::defOption {optDef option args} {
    ...
    if {([llength $args] % 2) == 1} {
      set msg "<<<Definition der Optionen hat Fehler\n"
      append msg "  Option: -$option  Argumente:<$args<\n"
      append msg "Müssen paarweise sein : -default abc -type xy\n"
      append msg "-default und -type kann fehlen"
      putError  $msg
    }

    process setupDef $args setup
    set def(-$option) [list $setup(-type) $setup(-default)]
    set oDefs(-$option) [list $setup(-type) $setup(-default)]
    return
}
...
proc ::pool::getopt::processOpt {optDef optlist optval} {
	...
	putError  "illegal number of item in optlist, must be even\n$optlist"
    }
...
proc ::pool::getopt::checkValues {optDef optVal} {
	...
	if {![[lindex $def($o) 0] $o $val($o)]} {
	    ...
	    putError  "invalid value given to option $o: $val($o)"
	}
    }
...
proc ::pool::getopt::parseArguments {optDef optList optVal} {
	...
	switch -glob -- $option {
      ...
      #puts stdout [::pool::dump::callstack getopt-error]
		putError  "unknown option: $option\n<$input>"
	    }
	    default {
...
proc ::pool::getopt::putError {msg} {
   ...
   puts "  Option\tDefault\tTypprüfung"
   foreach  opt [array names oDefs] {
      puts -nonewline "  $opt\t\t<[lindex $oDefs($opt) 1]>"
      puts     "\t<[lindex $oDefs($opt) 0]>"
