# 0.7 : Korrektur
# 0.6 : führende Nullen bei nextmonth
# 0.5 : führende Nullen bei Monat 
 package provide callib 1.0
 ...
 namespace eval callib {
    ...
    ##################################
        ...
        #tag
        set tag_num [clock format [clock scan now] -format "%d"]
        set calState($holder.day) [string trimleft $tag_num "0"]         
    ...
    ##################################################################
        variable calState
        set act_day   $calState($parent.day)
        set act_row {}
        set act_col {}
        ...
        # akt Tag markieren
        if {($act_row != {}) && ($act_row != {})} {
             $parent.$act_col$act_row configure -foreground red
        }
            ...
            "configure" {
                ...
                foreach {opt val} $args {
                    switch -- $opt {
                        ...
                        "-day" {
                            if {$val == ""} {
                                return $calState($parent.day)
                              };# END: if no month specified
                            if {($val > 0) && ($val < 32)} {
                                set calState($parent.month) $val
                              } else {
                                error "Month value must be between 1 and 31"
                              }
                            set calState($parent.clicked) {}  
                          }
                        "-month" {
            ...
            "nextmonth" {
                if {[llength $args]} {
                    error "nextmonth not allowed to have arguments"
					 ...
					 # führende Null ?
                set calState($parent.month) [_nooctal $calState($parent.month)]
 ...
 #---------------------------------------------------------------
   ...
   # _nooctal  int
   # int  : Integer mit oder ohne führende Null
   # Ergebnis:
