...
#
# 14.08.2020   fsdialog-1.17.4
#                xtoolsbin->binDirProd
# 
# 09.06.2019   fsdialog-1.17.3
#                tclkit_sparc -> tclkit
# 27.10.2018   fsdialog-1.17.2
#                xtoolBinRem -> xtoolsBin
# 28.07.2018   fsdialog-1.17.1
#    -xts entfernt, xsrcxm-Methode
# 04.07.2018   fsdialog-1.17.0  - xts-fähig
# 23.04.2018   fsdialog-1.16.1
#   - ttk::dialog::file::treeclose: Abfrage [txt tag range $n]
# 21.06.2016 1.16.0
#    -  -parent -> wv(w,top)
# 14.02.2015 1.15.0
#    -  multiselect und pattern bei open
# 08.01.2015  : ~ -> spusHome

package provide fsdialog 1.17.4
...
proc ::ttk::dialog::file::Create {win class} {
   ...
   if {$data(type) eq "open"} {
      $data(newBtn) configure -state disabled
   }
   ...
   if {$data(type) eq "open"} {
      # wenn -hosts leer, dann nur Label sonst Combobox
      if {$data(-hosts) eq ""} {
         set data(hostMenuBtn) [Label $f1.host \
            -relief sunken \
            -width 8 \
            -textvariable ::ttk::dialog::file::${dataName}(selectHost) \
            -text $data(-host) ]
      } else {
         set data(hostMenuBtn) [ComboBox $f1.host \
            -text $data(-host) \
            -values  $data(-hosts) \
            -modifycmd [list ::ttk::dialog::file::Update $win] \
            -editable 0 ]
      }
      set ::ttk::dialog::file::${dataName}(selectHost) $data(-host)
      $data(hostMenuBtn) configure \
         -textvariable ::ttk::dialog::file::${dataName}(selectHost)
   } else {
      set data(selectHost) localhost
      set data(-mithost) 0
   }
   ...
   if {$data(type) eq "open"} {
      pack $data(hostMenuBtn) -side left -fill x -expand 1 -padx 8
   }
...
#------------------------------------------------------------
proc ::ttk::dialog::file::dirRemote {w host} {
   # holt mit ssh Dirs und Files incl Details
   # vom remote Host
   # Erg: Matrix mit Dirs und Files und Details
   #
   set dataName [winfo name $w]
   upvar ::ttk::dialog::file::$dataName data
   global gd
   ...
   set cwd [lindex $data(history) $data(histpos)]
   set dir [file normalize $cwd]
   
   set pat *
   if {$data(-hidden)} {set pat {* .*}}
   
   # Trennen zwischen files und dir
   set filter $data(filter)
   if {$filter eq "*"} {set filter $pat}
   
   set pars  "$dir~$pat~$filter"
   ...
   if {[catch {::spec::specExec ssh spsy@$host \
            $gd(binDirProd)/tclkit $gd(binDirProd)/remdirservice.tcl \
            '$pars'} erg]} {
...
#------------------------------------------------------------
proc ::ttk::dialog::file::dirLocal {w} {
   # liefert lokal Dirs und Files incl Details
   # in einer Matrix
   #
   set dataName [winfo name $w]
   upvar ::ttk::dialog::file::$dataName data
   ...
   set mat [list]
   foreach name $dirs  {
      # bei Verzeichnissen . und .. unterdrücken
...
proc ::ttk::dialog::file::Update {w} {
   ...
   # lokal oder remote Aufruf ?
   if {[info exists data(selectHost)]} {
      set host $data(selectHost)
   } else {
      set host "localhost"
   }
   ...
   if {$host ne [info hostname] \
         && $host ne "localhost" } {
      if {[catch {::ttk::dialog::file::dirRemote $w $host} dirFiles]} {
         ::utils::gui::msg error "Fehler" $dirFiles
         $w configure -cursor ""
         return {}
      }
      # puts remote
   } else {
      # lokal DirInfos holen
      if {[catch {::ttk::dialog::file::dirLocal $w} dirFiles]} {
         ::utils::gui::msg error "Fehler" $dirFiles
         $w configure -cursor ""
         return {}
      }
   ...
   # dirFiles enthält dirs und files
   set dlist ""
   set flist ""
   foreach filTyp $dirFiles {
      lassign $filTyp name typ
      if {$typ eq "dir"} {
         lappend dlist $filTyp
      } else {
         lappend flist $filTyp
      }
   }
...
proc ::ttk::dialog::file::Done {w} {
   ...
   # Pattern ?
   if {[string first "*" $data(selectFile)] >= 0} {
      set data(filter) $data(selectFile)
      ::ttk::dialog::file::Update $w
      return
   }
   set cwd [lindex $data(history) $data(histpos)]
   set path ""
...
proc ::ttk::dialog::file::tkFDialog {type args} {
   ...
   if {$type ne "open"} {
      $data(newBtn) configure -state normal
   } else {
      $data(newBtn) configure -state disabled
   }
...
proc ::ttk::dialog::file::Config {dataName type argList} {
   ...
   if {[info exists wv(w,top)]} {
      if {[winfo exists $wv(w,top)]} {
         lappend specs [list -parent "" "" $wv(w,top)]
      } else {
         lappend specs [list -parent "" "" "."]
      }
   } else {
      lappend specs [list -parent "" "" "."]
   }
...
proc ::ttk::dialog::file::treeOpen {w path {index insert} {subdir .}} {
   ...
   if {[catch {glob -nocomplain -tails -directory $path -types d * .*} list]} {
      if {$img ne ""} {
