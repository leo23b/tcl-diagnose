...
proc ::canvas::gradient::DrawGradient {canvas args} {
   ...
   if {[$canvas cget -scrollregion] eq ""} {
      set canWidthPx  [winfo width  $canvas]
      set canHeightPx [winfo height $canvas]
   } else {
      # W und H aus Scrollbereich
      set canWidthPx  [lindex [$canvas cget -scrollregion] 2]
      set canHeightPx [lindex [$canvas cget -scrollregion] 3]
   }
...
package provide canvas::gradient 1.0
return